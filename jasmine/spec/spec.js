...
describe('Inverted Index Test', () => {
  ...
  describe('Returns a string without characters and all in lower case', () =>{
    it('should return a string of all in lower cases', () => {
      expect(invertedIndex.returnAsArrayOfWords('ABC.(D?RFG,HIJKL')).toEqual(['abcdrfghijkl']);
      expect(invertedIndex.returnAsArrayOfWords('ALICE IN, WONDERLAND')).toEqual([ 'alice', 'in', 'wonderland' ]);
    });
  });
  describe('Returns an array of non-repeting strings', () =>{
    it('should return an array of unique strings', () => {
      expect(invertedIndex.returnUniqueWords(['alice','alice'])).toEqual(['alice']);
      expect(invertedIndex.returnUniqueWords(['alice', 'alice', 'boy', 'girl', 'child','boy'])) === [ 'alice', 'boy', 'girl', 'child' ];
    });
  });
  describe('Returns an index', () =>{
    it('should return an index when given valid data', () => {
      let data = [{ 'title': 'Alice', 'text': 'how' }, { 'title': 'Rings', 'text': 'how' }];
        expect(invertedIndex.buildIndex(data)).toBe('Index Built');
    });
  });
  describe('Return an object for a given search parameters', () => {
    it('should take in a word and an indexed book and return matches for that word', () => {
      let newData = [{ 'title': 'Jane the virgin', 'text': 'rowland in' }];
      invertedIndex.buildIndex(newData);
      expect(invertedIndex.searchIndex('the')).toEqual({'the': [ 'Jane the virgin', ]});
      invertedIndex.buildIndex(mockData);
      expect(invertedIndex.searchIndex('the')).toEqual({'the': [ 'Jane the virgin', 'The Lord of the Rings: The Fellowship of the Ring.' ]});
    });
  });
});
